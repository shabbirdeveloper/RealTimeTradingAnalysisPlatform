  ...
  return (
    <div>
      <section className="relative overflow-hidden border-b border-border/70 bg-grid">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-[520px] bg-[radial-gradient(ellipse_760px_420px_at_50%_-10%,hsl(var(--primary)/0.16),transparent_65%)]" />
        <div className="container relative flex flex-col items-center gap-7 py-24 text-center md:py-32">
          ...
          <h1 className="max-w-3xl text-5xl font-semibold leading-[1.08] tracking-tight md:text-6xl">
      ...
      <section className="container grid grid-cols-1 gap-4 py-16 md:grid-cols-3">
        <FeatureCard icon={Filter} title="Quality over quantity" desc="Most analysis cycles end in NO TRADE. Only setups that clear technical, regime, and meta-model filters surface as A/A+/A++." />
        <FeatureCard icon={LineChart} title="Multi-timeframe confluence" desc="H4 macro context down to M5 entry timing — signals need alignment, not just one indicator crossing a line." />
        <FeatureCard icon={TrendingUp} title="Verified performance, not claims" desc="Overall accuracy and A++ accuracy are tracked separately from real recorded results. No hard-coded win rates." />
      </section>
...
function Stat({ value, label, accent }: { value: string; label: string; accent?: boolean }) {
  return (
    <div className="text-center">
      <p className={`font-mono-tabular text-2xl font-bold tracking-tight ${accent ? "text-aplusplus" : "text-foreground"}`}>{value}</p>
      <p className="text-[11px] uppercase tracking-wider text-muted-foreground">{label}</p>
    </div>
  );
}

function FeatureCard({ icon: Icon, title, desc }: { icon: typeof Filter; title: string; desc: string }) {
  return (
