  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Calculated from recorded demo results only — never a claimed live win rate.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        <Kpi icon={p.currentStreak.type === "WIN" ? Flame : Snowflake} label="Current streak" value={`${p.currentStreak.count} ${p.currentStreak.type}`} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <BreakdownCard title="Performance by confidence bucket" data={p.byConfidenceBucket} full />
      </div>
    </div>
  );
}

function BreakdownCard({ title, data, full }: { title: string; data: typeof p.byAsset; full?: boolean }) {
  return (
